            ...
            for other_golfers_tuple in itertools.combinations(remaining_pool_for_combination, S - 1):
                ...
                for i in range(S):
                    for j in range(i + 1, S):
                        ...
                        if pair in played_together_history: # Checks against *previous* weeks' pairs
                ...
                if is_valid_group:
                    # MAKE MOVE: Add this group to the current week's temporary schedule
...
if __name__ == "__main__":
    ...
    G_val = 8 # Number of groups (12 players / 4 golfers/group)
    ...
    pairs_per_week = G_val * ((S_val * (S_val - 1)) // 2)
    
    theoretical_max_W = total_unique_pairs // pairs_per_week
    print(f"Theoretical maximum possible weeks (if all pairs were unique across weeks): {theoretical_max_W}")
    ...
    while True:
        ...
        # Pass verbose=True to see week-by-week logs for any successful path
        solution = solve_social_golfer_dfs(G=G_val, S=S_val, W=current_W_to_test, verbose=True)
